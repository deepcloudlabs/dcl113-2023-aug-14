...
int main() {
    ...
    for (auto &entry: countries) {
        ...
        auto statisticsReducer = [](statistics<shared_ptr<city>, int> cityPopulationStatistics,
                                    shared_ptr<city> &a_city) -> statistics<shared_ptr<city>, int> {
            ...
            if (cityPopulationStatistics.min > cityPopulation) {
                cityPopulationStatistics.min = cityPopulation;
                cityPopulationStatistics.min_element = a_city;
            } else if (cityPopulationStatistics.max < cityPopulation) {
                cityPopulationStatistics.max = cityPopulation;
                cityPopulationStatistics.max_element = a_city;
            }
            return cityPopulationStatistics;
        };
        auto populationStatistics = accumulate(country_cities.begin(), country_cities.end(),
                                               statistics<shared_ptr<city>, int>(), statisticsReducer);
